...
param(
  ...
  [switch]$NoOwnerNoAcl = $true,

  [switch]$CleanIfExists = $true,

  [string]$CloudSslMode = "require",
...
if ($Mode -in @("DumpAndRestore","DumpOnly")) {
  $noOwnerAclArgs = @()
  if ($NoOwnerNoAcl) { $noOwnerAclArgs += @("--no-owner","--no-acl") }
  if ($UseDockerClient) {
    $cmd = @(
      "pg_dump",
      "--dbname","\"$LocalDbUrl\"",
    ...
    if ($NoOwnerNoAcl) { $cmd += @("--no-owner","--no-acl") }
    $cmd += @("--verbose")
    Run-InDocker -Cmd ($cmd -join ' ')
  } else {
    Write-Host ">> pg_dump to $DumpPath"
    & pg_dump --dbname $LocalDbUrl --format c --file $DumpPath @noOwnerAclArgs --verbose
...
if ($Mode -eq "Stream") {
  if ($UseDockerClient) {
    $cmd = @(
      "set -o pipefail;",
      "pg_dump --dbname \"$LocalDbUrl\" --format=p",
      ($NoOwnerNoAcl ? "--no-owner --no-acl" : ""),
      "--verbose",
      "| psql \"$CloudDbUrl\""
    ) -join ' '
    ...
    $dumpArgs = @("--dbname", $LocalDbUrl, "--format", "p", "--verbose")
    if ($NoOwnerNoAcl) { $dumpArgs += @("--no-owner","--no-acl") }
    $pgDump = Start-Process -FilePath "pg_dump" -ArgumentList $dumpArgs -NoNewWindow -PassThru -RedirectStandardOutput pipe
    throw "Streaming mode is not supported without intermediate file in this shell setup. Use DumpAndRestore or run via Docker client."
  }
}
...
if ($Mode -in @("DumpAndRestore","RestoreOnly")) {
  ...
  if ($UseDockerClient) {
    $parts = @("pg_restore","--dbname","\"$CloudDbUrl\"")
    if ($CleanIfExists) { $parts += @("--clean","--if-exists") }
    if ($NoOwnerNoAcl) { $parts += @("--no-owner","--no-acl") }
    if ($ParallelJobs -gt 1) { $parts += @("-j", "$ParallelJobs") }
    $parts += @("--verbose","\"$DumpPath\"")
    Run-InDocker -Cmd ($parts -join ' ')
  } else {
    Write-Host ">> pg_restore to cloud"
    $args = @("--dbname", $CloudDbUrl)
    if ($CleanIfExists) { $args += @("--clean","--if-exists") }
    if ($NoOwnerNoAcl) { $args += @("--no-owner","--no-acl") }
    if ($ParallelJobs -gt 1) { $args += @("-j", "$ParallelJobs") }
    $args += @("--verbose", $DumpPath)
    & pg_restore @args
